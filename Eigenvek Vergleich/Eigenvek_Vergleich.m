%% 粗网格特征向量处理
% 粗网格特征向量文件前缀和文件数量
file_ELEMENT_prefix = 'Gitter_5mm_Eigenvek_';
file_ELEMENT_suffix = '.txt';
num_ELEMENT_files = 50;
...
% 细网格特征向量文件前缀和文件数量
file_element_prefix = 'Gitter_3mm_Eigenvek_';
file_element_suffix = '.txt';
num_element_files = 50;
...
MAC_quote = zeros(num_element_files);
for i = 1:1:num_element_files
    for j = 1:1:num_element_files
        MAC_quote(i, j) = (dot(element_eigenvec_interp(i, :), ELEMENT_eigenvectors(j, :)))^2/dot(element_eigenvec_interp(i, :), element_eigenvec_interp(i, :))/dot(ELEMENT_eigenvectors(j, :), ELEMENT_eigenvectors(j, :));
        fprintf('MAC矩阵第%d行, %d列元素已被计算完成\n', i, j);
    end
end

%Visualisierung
imagesc(MAC_quote);          % 显示矩阵图像
